...
def show_point_clouds(
    ...
    pcd2.points = o3d.utility.Vector3dVector(points2 + offset)
    if colors:
        pcd.colors = o3d.utility.Vector3dVector(points_colors[:, :3])
        pcd2.colors = o3d.utility.Vector3dVector(points_colors2[:, :3])
    mesh_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(
        size=1.6, origin=[0, 0, 0]
    )
...
def show2(
    occ_state, occ_show, occ_state2, occ_show2, voxel_size, vis=None, offset=[0, 0, 0]
):
    # occ_state, voxel_size = data['occ_state'].cpu(), data['voxel_size']
    colors = colormap_to_colors / 255
    pcd, labels, occIdx = voxel2points(occ_state, occ_show, voxel_size)
    _labels = labels % len(colors)
    pcds_colors = colors[_labels]
    pcd2, labels2, occIdx2 = voxel2points(occ_state2, occ_show2, voxel_size)
    _labels2 = labels2 % len(colors)
    pcds_colors2 = colors[_labels2]
    bboxes = voxel_profile(pcd, voxel_size)
    bboxes_corners = my_compute_box_3d(bboxes[:, 0:3], bboxes[:, 3:6], bboxes[:, 6:7])
    # bboxes_corners = torch.cat([my_compute_box_3d(box[0:3], box[3:6], box[6:7])[None, ...] for box in bboxes], dim=0)
    bases_ = torch.arange(0, bboxes_corners.shape[0] * 8, 8)
...
def vis_occ(vox_occ, mask_camera, mask_lidar, labels):
    vox_occ = vox_occ.copy()
    vox_occ = mask_sky(vox_occ)
    vox_occ = mask_ego_car(vox_occ)
    vox_occ[vox_occ != 17] = 1
    voxel_size = [0.1, 0.1, 0.1]
    free_label = 17
    voxel_label_vis = vox_occ
    voxel_show = np.logical_and(mask_camera, vox_occ != free_label)
    # voxel_show = vox_occ != free_label
    vis = main(
...
def vis_occ_sem(voxel_semantics, mask_camera, mask_lidar, labels):
    voxel_semantics = voxel_semantics.copy()
    voxel_semantics = mask_sky(voxel_semantics)
    voxel_semantics = mask_ego_car(voxel_semantics)
    # voxel_semantics = labels[0]
    voxel_size = [0.1, 0.1, 0.1]
    free_label = 17
    voxel_label_vis = voxel_semantics
    voxel_show = np.logical_and(mask_camera, voxel_semantics != free_label)
    # voxel_show = voxel_semantics != free_label
    vis = main(
        torch.from_numpy(voxel_label_vis),
        torch.from_numpy(voxel_show),
        voxel_size=voxel_size,
