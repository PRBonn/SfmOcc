...
class PointToMultiViewDepth(object):
    ...
    def __call__(self, results):
        ...
        # project lidar points into each image
        # apply augmentations first: rotation and translation
        imgs, rots, trans, intrins = results["img_inputs"][:4]
        post_rots, post_trans, bda = results["img_inputs"][4:]
        depth_map_list = []
        # only iterate over the first 6 images (current ts)
        for cid in range(len(results["cam_names"])):
            ...
            ##
            ##TODO: if sfm, load here the sfm image and then apply the same transformation
            # there will be errors because my coordinates are already int
            # apply augmentations
            points_img = (
...
class PrepareImageInputs(object):
    ...
    def get_inputs(self, results):
        ...
        for cam_name in cam_names:
            ...
            all_img_augs[cam_name] = img_augs
...
@PIPELINES.register_module()
class GenerateDepthSup(object):
    """Generate depth supervision given depth image and applied augmentations"""

    def __init__(self, data_config):
        self.cams = data_config["cams"]
        self.input_size = data_config["input_size"]
        self.src_size = data_config["src_size"]

    def __call__(self, results):
        all_img_augs = results["img_augs"]
        curr_depth = results["curr_depth"]
        # load depth supervision
        depths, coors = curr_depth

        # apply same augmentations
        H, W = self.src_size
        dimgs = []
        for cam, coor, depth in zip(self.cams, coors, depths):  # iterate for each cam
            resize, resize_dims, crop, flip, rotate = all_img_augs[cam]
            newW, newH = resize_dims
            scale_factors = np.array([newH / H, newW / W])
            # resize
            coor2 = (coor * scale_factors).astype(np.int16)
            res_img = np.zeros((resize_dims[1], resize_dims[0]), dtype=np.float32)
            res_img[coor2[:, 1], coor2[:, 0]] = depth
            # crop
            crop_dimg = res_img[crop[1] : crop[3], crop[0] : crop[2]]
            if flip:
                crop_dimg = np.fliplr(crop_dimg)
            # rotate
            rot_dimg = np.array(Image.fromarray(crop_dimg).rotate(rotate))
            dimgs.append(rot_dimg)
        dimgs = np.stack(dimgs, axis=0)
        results["gt_depth"] = dimgs
        return results
